...
CP_BSPNode* gb_buildBSPTree(vector<CP_Partition*> &vp, CP_BSPNode* parent, char childInfo) {
	...
	if(vp.size() > 0) // Q : 捞 炼扒捞 H assign 窍瘤 傈栏肺 甸绢啊具 窍瘤 臼唱?
		tree->pos_coincident.push_back(H); // 泅犁 畴靛狼 hyperplane (捞霸 partitionLine push_back 窍扁 傈俊 甸绢啊搁 恐 巩力啊 登唱?)

	// 泅犁 sub tree(畴靛)俊 巢酒乐绰 葛电 颇萍记甸阑 H俊 classification 窍绊, H肺 肋扼霖促.
	for(const auto &p : vp){
	...
	// H肺 Tree甫 Cut茄 搬苞俊 蝶扼 窍困 畴靛甫 父甸绢 霖促.
	if(F_left.size() == 0){
...
// partition 捞 T狼 郴何 康开俊 粮犁窍绰瘤 八荤茄促.
bool gb_p_in_region(CP_BSPNode* T, CP_Partition* partition, CP_Point &begin, CP_Point& end, const CP_Point &cross, 
	...
	while(node->parent != NULL){
		...
		if(child == node->leftChild){ 
			// 父距 泅犁 畴靛啊 parent 扁霖 剧狼 康开俊 乐绰 版快..
			t_bp->begin = node->partition->begin;
			t_bp->end = node->partition->end;
		}
		else{
			// 父距 泅犁 畴靛啊 parent 扁霖 澜狼 康开俊 乐绰 版快..
			// Q : 恐 官层林瘤? -> vector狼 规氢阑 官操绢辑 郴/寇何 八荤?
		...
		if(compare_float(cross_product_tp, 0)){
			...
			if(t.cross_product(v) >= 0) {
				...
				// partition捞 T 官冰俊 乐绰 巴捞 犬角窍骨肺 歹捞惑 柳青且 鞘夸啊 绝澜.
				return false;
			}
		}

		// projective/homogeneous equation栏肺 背痢 备窍扁 (3瞒盔俊辑绰 背急 备窍扁啊 登绢具 窃)
		point.m_x =  (tc * pb - tb * pc) / (tb * pa - ta * pb);
		point.m_y =  (tc * pa - ta * pc) / (tb * pa - ta * pb);

		// 父距 滴 俺狼 氦磐啊 乞青窍瘤 臼篮 版快...
...
bool gb_t_in_region(CP_BSPNode* T, CP_Partition* partition, CP_Point &pos, CP_Point *cross, 
	...
	while(node->parent != NULL){
		...
		if((-tb) * pa - (-pb) * ta >= -TOLERENCE && (-tb) * pa - (-pb) * ta  <= TOLERENCE){
			//平行 现在假定重合或者平行在Tnode-partition左边都可以
